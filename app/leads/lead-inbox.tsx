    ...
    <section className="border-t border-black/10 bg-[#17201c] p-6 text-white">
      <div className="grid gap-6 lg:grid-cols-[.7fr_1.3fr]">
        ...
        <form onSubmit={createPaymentLink} className="grid gap-3 sm:grid-cols-2 xl:grid-cols-6 xl:items-end">
          <label className="grid gap-2 text-[10px] font-bold uppercase tracking-[.12em] text-white/55">Exact agreed amount<input className="min-h-11 border border-white/20 bg-white/10 px-3 text-sm font-medium normal-case tracking-normal text-white" type="number" inputMode="decimal" min="50" step="0.01" value={amount} onChange={(event) => { setAmount(event.target.value); setState('idle'); }} placeholder="No default" disabled={!qualified} /><span className="text-[9px] font-normal normal-case leading-4 tracking-normal text-white/45">Enter the milestone from the signed agreement.</span></label>
