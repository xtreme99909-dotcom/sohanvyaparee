...
assert(Boolean(read('.github/workflows/qa-gates.yml')), 'QA workflow is present.');
assert(Boolean(read('qa/project.json')), 'Active QA manifest is present.');

const nextConfig = read('next.config.ts');
assert(/headers\s*\(/.test(nextConfig), 'Application security-header function exists.');
for (const header of ['Strict-Transport-Security','X-Content-Type-Options','X-Frame-Options','Referrer-Policy','Permissions-Policy']) {
  assert(nextConfig.includes(header), header + ' is configured at application level.');
}
if (!/Content-Security-Policy/i.test(nextConfig)) warn('No source-level Content-Security-Policy was detected; add only after nonce and third-party compatibility are runtime-tested.');
